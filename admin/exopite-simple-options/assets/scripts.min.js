jQuery('.swell-sync').on('click',function(){
	console.log( jQuery(this).attr("data-sync") );
	var sync = jQuery(this).attr("data-sync");
	jQuery('.button_sync_'+sync).show();
	var ajaxphp = '/wp-admin/admin-ajax.php';
	jQuery.ajax({
		type: 'POST',
	    url: ajaxphp,
	    dataType: 'json',
	    data:{
		    	action: "swell_get_data",
		    	syncData : sync, 
	    	},
	    success: function( ) {
	    	jQuery('.button_sync_'+sync).hide();
	    	jQuery('.swell_sync_success_'+sync).html( sync+" Synced Successfully" ).show().delay(5000).fadeOut();
	    }
	});
