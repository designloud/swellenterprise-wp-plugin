...
//				$this->config = wp_parse_args( $this->config, $default );
		...
		public function loop_fields( $callbacks ) {
			...
			foreach ( $this->fields as $section ) {
				...
				// after
				if ( $callbacks['after'] ) {
					call_user_func( array( $this, $callbacks['after'] ), $section );
				}
			}
		...
		public function add_field( $field, $value = null ) {
			...
			if ( isset( $field['title'] ) ) {
				if( isset( $field['id'])&&  $field['id'] && 
					(isset($field['post_type']) && $field['post_type'] === 'task') 
					|| 
					(isset($field['post_type']) && $field['post_type'] === 'note') ) {
					if( isset( $field['id'] ) ) {
						$id = $field['id'];
						$output .= '<div class="items">';
						$output .= '<a class="clearfix edit-items" href="'.get_site_url().'/wp-admin/post.php?post='.$field['id'].'&action=edit">Edit</a>';
						$output .= '</div>';
					}
			...
			if ( ! isset( $field['pseudo'] ) || ! $field['pseudo'] ) {
				//$output .= '<div class="clearfix">'.print_r($field).'</div>';
				/*if( $field['id'] && $field['post_type'] === 'task' || $field['post_type'] === 'note') {
					$id = $field['id'];
					$output .= '<div class="items">';
					$output .= '<a class="clearfix view-items" href="'.get_permalink($id) .'">View</a>';
					$output .= '<a class="clearfix edit-items" href="'.get_site_url().'/wp-admin/post.php?post='.$field['id'].'&action=edit">Edit</a>';
					$output .= '</div>';
				}*/
